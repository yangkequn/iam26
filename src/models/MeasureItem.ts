...
export class MeasureItem implements IMeasureItem {
    ...
    public static From(obj: object): MeasureItem {
    ...
    public Load = (setState) => this.LoadCheck()&&Load(`/api/measure?id=${this.measureId}`,this, setState)
    ...
    private static ListTransformer (data: string[]): MeasureItem[] {
        if (!data|| !data["list"]) return [];
        return data["list"].map((id: string): MeasureItem => new MeasureItem(id, "", "", "",  0,0, false ))
    } 
    public static MyList(setState) {
        Get(`/api/measureList`, setState, MeasureItem.ListTransformer, null, false)
    }
    public  static  Recommend = (text, setState) => Get( `/api/measureRecommend?text=${text}`, setState, MeasureItem.ListTransformer,null,false)
