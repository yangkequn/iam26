...
export class GoalItem implements IGoalItem {
    ...
    public static From(obj: object): GoalItem {
    ...
    public Load = (setState) => this.LoadCheck()&&Load(`${GoalItem.Url}?id=${this.id}`,this, setState)
    ...
    private static ListTransformer (data: string[]): GoalItem[] {
        if (!data|| !data["list"]) return [];
        return data["list"].map((id: string): GoalItem => new GoalItem(id, "", "", "", "", 0, false ))
    }        
    public static Recommend(TextRequest, setState) {
        Get(`/api/goalRecommend?text=${TextRequest}`, setState, GoalItem.ListTransformer, null, false)
    }
    public static MyList(setState) {
        Get(`/api/goalList`, setState, GoalItem.ListTransformer, null, false)
    }
