...
type (
	actListModel interface {
		...
		FindOne(ctx context.Context, id string) (*ActList, error)
...
func newActListModel(conn sqlx.SqlConn) *defaultActListModel {
...
func (m *defaultActListModel) FindOne(ctx context.Context, id string) (*ActList, error) {
...
func (m *defaultActListModel) Delete(ctx context.Context, id string) error {
	query := fmt.Sprintf("delete from %s where id = $1", m.table)
	_, err := m.conn.ExecCtx(ctx, query, id)
	return err
}

func (m *defaultActListModel) tableName() string {
	return m.table
}
